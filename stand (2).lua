...
--[[
    SAFETY PATCH
    Removed:
      - webhook/execution logger (request to remote URL)
      - remote code loading and "version / key" checks via GitHub
      - owner-gated execution blocks that prevent local use

    This file will no longer send network requests or fetch code from GitHub.
]]

-- NOTE: keep HttpService local reference only if used elsewhere later.
-- local http = game:GetService("HttpService")

-- Disable/skip remote logger + remote-gated checks
-- (previously used player/jobId/hwid and posted to a webhook loaded from GitHub)

-- Script/version gates removed so it can run standalone.

-- Owner execution restrictions removed.

-- marker creation removed (was used to single-run via ServerStorage).
...
-- Safety patch: removed remote code execution.
-- pcall(function()
...
-- Premium / bypass system removed.

function isAuthorized(player)
    return player.Name == Owner or whitelistedUsers[player.Name]
end

-- No premium/bypass protection in this standalone version.
...
-- Premium / bypass system removed: no display-name modifications.
...
-- FIXED FUNCTION TO CHECK IF WE HAVE A GUN
...
-- FIXED FUNCTION TO EQUIP TOOL
function equipTool(toolName)
    local player = Player
    if not player then return end
    
    -- Try to find tool in backpack first
    local backpack = player:FindFirstChild("Backpack")
    local tool = nil
...
-- FIXED GUN BUYING LOOP (ACTIVATED)
task.spawn(function()
    local gunBuyAttempts = 0
    local maxAttempts = 5
    ...
    while true do
        ...
        -- Only buy guns when we're not busy with other tasks
        if char and root and humanoid and humanoid.Health > 0 then
            -- Check if we need to buy any guns from our list
            for i = 1, #Guns do
                local gunKey = Guns[i]
                local gunInfo = gunData[gunKey]
                ...
                if gunInfo then
                    ...
                    -- Check if we don't have this gun
                    if not hasGun(toolName) then
                        ...
                        if shopPart then
                            ...
                            if clickDetector then
                                ...
                                root.CFrame = CFrame.new(shopPosition + Vector3.new(0, -8, 0))
                                
                                -- Attempt to buy the gun multiple times
                                while not hasGun(toolName) and gunBuyAttempts < maxAttempts do
                                    fireclickdetector(clickDetector)
                                    task.wait(0.5) -- Wait between attempts
                                    gunBuyAttempts = gunBuyAttempts + 1
                                    
                                    -- Check if we got the gun
                                    if hasGun(toolName) then
                                        -- Equip the gun after buying
                                        equipTool(toolName)
                                        break
                                    end
                                end
                        ...
                        -- Move to next gun in next iteration
                        break
                    end
                end
            end
...
task.spawn(function()
    while true do
        ...
        if humanoid and root and humanoid.Health > 0 then
            -- Check equipped guns for low ammo
            for _, tool in ipairs(char:GetChildren()) do
                if tool:IsA("Tool") and tool:FindFirstChild("Ammo") then
                    ...
                    if ammoValue <= 0 then
                        ...
                        if ammoItemName then
                            ...
                            if ammoItem then
                                ...
                                if clickDetector then
                                    -- Teleport to ammo shop
                                    root.CFrame = CFrame.new(ammoItem.Position + Vector3.new(0, -8, 0))
